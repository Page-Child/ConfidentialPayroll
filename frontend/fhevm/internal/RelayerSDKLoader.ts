...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        console.log("[RelayerSDKLoader] script onerror from CDN, try local fallback... ");
        ...
        localScript.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            console.log("[RelayerSDKLoader] local script onload FAILED...");
            reject(
              new Error(
                `RelayerSDKLoader: Relayer SDK script has been successfully loaded from ${SDK_LOCAL_URL}, however, the window.relayerSDK object is invalid.`
              )
            );
            return;
          }
        ...
        localScript.onerror = () => {
          reject(
            new Error(
              `RelayerSDKLoader: Failed to load Relayer SDK from both ${SDK_CDN_URL} and ${SDK_LOCAL_URL}`
            )
          );
        };
...
function isFhevmRelayerSDKType(
  ...
  if (!objHasProperty(o, "ZamaEthereumConfig", "object", trace)) {
    trace?.("RelayerSDKLoader: relayerSDK.ZamaEthereumConfig is invalid");
    return false;
  }
